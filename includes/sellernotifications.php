...
if(htmlspecialchars($_SERVER['REQUEST_METHOD'])=="POST"){
    if(isset($_POST['lastinterestid'])){
        ...
        if($id=="0"){
         $query="SELECT `interest_id`, `interested_user_id`,`Date`, `property_id` FROM `interested` WHERE `property_owner_id`='$userid' AND `canceled` = 0 AND interest_id >= $id ORDER BY `Date` DESC";     
         $Result=$database->query($query);
                while($res=$database->fetchArray($Result)){
               $user->setId($res['interested_user_id']);
               $property->setId($res['property_id']);
                $result.="<tr><td>".$property->getName()."</td><td>".$property->getType()."</td><td><a href='profile.php?user_id='".$res['interested_user_id']."'>".$user->getUname()."</a></td><td>".$res['Date']."</td><td><button id='accept' onclick=\"accept('".$res['interest_id']."');\">Accept!</button></td><td><button id='reject' onclick=\"reject('".$res['interest_id']."');\">Reject!</button></td><td><button id='init' onclick=\"initmessage('".$res['interest_id']."');\">Message!</button></td>"."</tr>";
